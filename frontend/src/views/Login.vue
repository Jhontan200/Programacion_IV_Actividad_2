...
const handleLogin = async () => {
  if (!email.value || !password.value) {
    Swal.fire({ icon: 'warning', title: 'Atención', text: 'Ingresa correo y contraseña.' });
    return;
  }
  ...
  try {
    ...
    // Emitir el usuario al componente padre
    ...
    Swal.fire({
      ...
      text: error.response?.data?.error || 'Credenciales inválidas.'
    });
  } finally {
...
<template>
  <div class="min-h-screen bg-slate-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
    <div class="sm:mx-auto sm:w-full sm:max-w-md text-center">
      <div class="inline-flex p-3 bg-blue-600 rounded-xl mb-4 shadow-lg text-white">
        <!-- Icono Escudo/Acceso -->
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-8 h-8">
          <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
        </svg>
      </div>
      <h2 class="text-3xl font-extrabold text-white">Sistema de Gestión de Ingreso</h2>
      <p class="mt-2 text-sm text-slate-400">Control de operaciones y proyectos constructores.</p>
    </div>

    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md p-4">
      <div class="bg-white py-8 px-6 shadow-2xl rounded-xl border border-slate-800">
        <form @submit.prevent="handleLogin" class="space-y-6">
          ...
          <div>
            ...
            <div class="relative">
              <input 
                ...
                required
                class="w-full pl-3 pr-10 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                placeholder="••••••••"
              />
              <button 
                type="button"
          ...
          <button 
            type="submit" 
            :disabled="isLoading"
            class="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-md text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 transition"
          >
